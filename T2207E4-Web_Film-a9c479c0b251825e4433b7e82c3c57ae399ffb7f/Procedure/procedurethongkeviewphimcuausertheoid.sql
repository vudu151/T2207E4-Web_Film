DROP PROCEDURE IF EXISTS sp_account_movie_view_statsbyuser;
CREATE PROCEDURE sp_account_movie_view_statsbyuser(
  IN start_date DATE,
  IN end_date DATE,
  IN account_ids BINARY(16)
)
BEGIN
  SELECT
    a.username,
  ...
  WHERE vs.date_stop_watch BETWEEN start_date AND end_date
       AND a.id = account_ids
  GROUP BY a.username, m.name, ws.name
  ORDER BY total_views DESC;
END;

CALL sp_account_movie_view_statsbyuser('2023-01-01', '2023-12-31', UNHEX('1a2b3c4d5e6f7g8h'));