...
BEGIN
    ...
    SET totalPages = CEIL(totalRecords / CAST(pageSize AS DECIMAL));   
     SELECT 
        m.id, m.is_popular, m.is_series, m.keywords, m.level, m.mmpa_rating, m.name, m.poster, m.release_date, m.screening_status, m.status, m.total_view, m.trailer ,g.name AS genremain, cm.name as catename,
